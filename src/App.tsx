...
import IndexFingerPointingPage from './pages/IndexFingerPointingPage'
import NavBar from './components/NavBar'

type Route = '/' | '/hand-tracking' | '/eye-tracking' | '/index-finger-pointing'

function getRoute(): Route {
  const path = window.location.pathname
  if (path === '/hand-tracking') return '/hand-tracking'
  if (path === '/eye-tracking') return '/eye-tracking'
  if (path === '/index-finger-pointing') return '/index-finger-pointing'
  return '/'
}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', width: '100%', height: '100%' }}>
      ...
      <div style={{ flex: 1, overflow: 'hidden' }}>
        ...
        {route === '/index-finger-pointing' && <IndexFingerPointingPage />}
      </div>
    </div>
  )
}
