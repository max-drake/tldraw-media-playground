...
class SelectionForegroundOverlayUtil extends import_editor.OverlayUtil {
  ...
  _getResizeHandleGeometry(handle, state, transform) {
    if (handle === "top" || handle === "bottom" || handle === "left" || handle === "right") {
      ...
      return new import_editor.Edge2d({
        start: import_editor.Mat.applyToPoint(transform, edge.start),
        end: import_editor.Mat.applyToPoint(transform, edge.end)
      });
    }
  ...
  _getEdgeLocalPoints(edge, width, height) {
    switch (edge) {
      case "top":
        return { start: new import_editor.Vec(0, 0), end: new import_editor.Vec(width, 0) };
      case "right":
        return { start: new import_editor.Vec(width, 0), end: new import_editor.Vec(width, height) };
      case "bottom":
        return { start: new import_editor.Vec(0, height), end: new import_editor.Vec(width, height) };
      case "left":
        return { start: new import_editor.Vec(0, 0), end: new import_editor.Vec(0, height) };
    }
  }
