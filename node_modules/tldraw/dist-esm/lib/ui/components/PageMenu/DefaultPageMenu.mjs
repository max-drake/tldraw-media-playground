...
const DefaultPageMenu = memo(function DefaultPageMenu2() {
  ...
  useEffect(() => {
    ...
    editor.timers.requestAnimationFrame(() => {
      const doc = editor.getContainerDocument();
      const elm = doc.querySelector(`[data-pageid="${currentPageId}"]`);
      if (elm) {
  ...
  const handleCreatePageClick = useCallback(() => {
    ...
    editor.run(() => {
      ...
      editor.timers.requestAnimationFrame(() => {
        const doc = editor.getContainerDocument();
        const elm = doc.querySelector(`[data-pageid="${newPageId}"]`);
        if (elm) {
          elm.querySelector("button")?.focus();
        }
      });
